...
extension Document {
  public func throwExceptionOnFailure(msg: String, portal: Portal) throws {
    var errors = [String]()
    ...
    do {
      if let lastScript = try self.select("script[type='text/javascript']").last() {
        ...
        if let regex = try? NSRegularExpression(pattern: failReason[portal]!, options: []) {
          let range = NSRange(
            location: 0,
            length: lastScript.data().trimmingCharacters(in: .whitespacesAndNewlines).count)
          if let match = regex.firstMatch(
            in: lastScript.data().trimmingCharacters(in: .whitespacesAndNewlines), options: [],
            range: range)
          {
            let reasonRange = match.range(at: 1)
            let reason =
              (lastScript.data().trimmingCharacters(in: .whitespacesAndNewlines) as NSString)
              .substring(with: reasonRange)
