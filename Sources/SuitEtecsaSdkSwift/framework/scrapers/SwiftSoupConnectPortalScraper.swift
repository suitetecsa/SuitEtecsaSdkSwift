...
public class SWiftSoupConnectPortalScraper : ConnectPortalScraper {
    ...
    public func parseNautaConnectInformation(html: String) -> Result<NautaConnectInformation, Error> {
        let keys = ["account_status", "credit", "expiration_date", "access_areas", "from", "to", "time"]
        ...
        if let doc = try? SwiftSoup.parse(html) {
            do {
                ...
                let accountInfo = AccountInfo(accessAreas: info["access_areas"]!, accountStatus: info["account_status"]!, credit: info["credit"]!, expirationDate: info["expiration_date"]!)

                try doc.select("#sesiontraza > tbody > tr").forEach { tr in
                    var connection: [String: String] = [:]
                    var index = 0
                    try tr.select("td").forEach { td in
                        connection[keys[index + 4]] = try td.text().trimmingCharacters(in: .whitespacesAndNewlines)
                        index += 1
                    }

                    lastsConnections.append(LastsConnection(from: connection["from"]!, to: connection["time"]!, time: connection["to"]!))
                }
                return Result.success(NautaConnectInformation(lastsConnections: lastsConnections, accountInfo: accountInfo))
            } catch NautaException.genery(let message) {
                return Result.failure(NautaException.genery(message: message))
            } catch {
                return Result.failure(NautaException.genery(message: error.localizedDescription))
