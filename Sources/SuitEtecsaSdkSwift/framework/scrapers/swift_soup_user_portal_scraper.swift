...
public class SwiftSoupUserPortalScraper: UserPortalScraper {
    ...
    private func getUserAttr(elementHtml: Element) -> String {
        if let attr = try? elementHtml.select("p").first()!.text().trimmingCharacters(in: .whitespacesAndNewlines) {
            return attr
        } else {
            return ""
        }
    ...
    public func parseNautaUser(html: String) -> Result<NautaUser, Error> {
        do {
            ...
            // Manejar el error
            print("Error: \(error.localizedDescription)")
            return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
        }
    }
        ...
        do {
            ...
            return Result.success(
                ConnectionsSummary(
                    ...
                    yearMonthSelected: try attrs[0].select("input[name=year_month_selected]").first()!.attr("value"), 
                    totalTime: try attrs[1].select(".card-stats-number").first()!.text().trimmingCharacters(in: .whitespacesAndNewlines), 
                    totalImport: try attrs[2].select(".card-stats-number").first()!.text().trimmingCharacters(in: .whitespacesAndNewlines),
                    uploaded: try attrs[3].select(".card-stats-number").first()!.text().trimmingCharacters(in: .whitespacesAndNewlines), 
                    downloaded: try attrs[4].select(".card-stats-number").first()!.text().trimmingCharacters(in: .whitespacesAndNewlines), 
                    totalTraffic: try attrs[5].select(".card-stats-number").first()!.text().trimmingCharacters(in: .whitespacesAndNewlines)
                )
            )
        } catch Exception.Error(_, let message) {
            print(message)
        ...
        do {
            ...
            return Result.success(
                TransfersSummary(
                    count: Int(try attrs[0].select("input[name=count]").first()!.attr("value"))!, 
                    yearMonthSelected: try attrs[0].select("input[name=year_month_selected]").first()!.attr("value"), 
                    totalImport: try attrs[1].select(".card-stats-number").first()!.text().trimmingCharacters(in: .whitespacesAndNewlines)
                )
            )
        } catch Exception.Error(_, let message) {
            print(message)
