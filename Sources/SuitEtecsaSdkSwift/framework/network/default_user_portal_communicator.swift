...
public class DefaultUserPortalCommunicator: UserPortalCommunicator {
    ...
    private func makeUrl(
        ...
        if !getAction {
            ...
            let url = "\(userPortalUrlBase)\((userPortalUrls[action]! as! [String : String])[subAction!]!)"
            switch subAction! {
                case "base":
                ...
                case "list":
                    ...
                    return Result.success(page != nil ? "\(url)\(yearMonthSelected!)/\(count!)/\(page!)" : "\(url)\(yearMonthSelected!)/\(count!)")
            ...
            case .success(let url):
                ...
                switch response {
                    ...
                    case .failure(let error):
                        return Result.failure(NautaException.getInformationException(message: "Fail to obtain csrf token :: \(error.localizedDescription)"))
                }
            case .failure(let error):
                return Result.failure(NautaException.getInformationException(message: "Fail to obtain csrf token :: \(error.localizedDescription)"))
        }
    ...
    public func login(username: String, password: String, captchaCode: String) async -> Result<String, Error> {
        let action = Action.Login
        let url = makeUrl(action: action)

        switch url {
            case .success(let url):
                let response = await session.post(url: URL(string: url)!, parameters: [
                    "csrf" : self.csrf,
                    "login_user": username,
                ...
                switch response {
                    case .success(let data):
                        return Result.success(String(data: data, encoding: .utf8) ?? "")
                    case .failure(let error):
                        return Result.failure(NautaException.loginException(message: "Fail to login :: \(error.localizedDescription)"))
                }
            case .failure(let error):
                return Result.failure(NautaException.loginException(message: "Fail to login :: \(error.localizedDescription)"))
        }
    ...
    public func topUp(rechargeCode: String) async -> Result<String, Error> {
        ...
        switch url {
            case .failure(let error):
                return Result.failure(NautaException.topUpException(message: "Fail to top up :: \(error.localizedDescription)")) 
            case .success(let url):
                let response = await session.post(url: URL(string: url)!, parameters: ["csrf": self.csrf, "recharge_code": rechargeCode, "btn_submit": ""])

                switch response {
                    case .success(let data):
                        return Result.success(String(data: data, encoding: .utf8) ?? "")
                    case .failure(let error):
                        return Result.failure(NautaException.topUpException(message: "Failt to top up :: \(error.localizedDescription)"))
                }
        }
    }

    public func transfer(amount: Float, password: String, destinationAccount: String? = nil) async -> Result<String, Error> {
        var parameters = [
            "csrf": self.csrf, 
            "transfer": String(format: "%.2f", arguments: [amount]).replacingOccurrences(of: ".", with: ","),
            "password_user": password,
            "action": "checkdata"
        ]
        ...
        switch makeUrl(action: Action.Transfer) {
            case .failure(let error): return Result.failure(NautaException.transferException(message: error.localizedDescription))
            case . success(let url):
    ...
    public func changePassword(oldPassword: String, newPassword: String) async -> Result<String, Error> {
        let parameters = [
            "csrf": self.csrf,
        ...
        switch makeUrl(action: Action.ChangePassword) {
            case .failure(let error): return Result.failure(NautaException.changePasswordException(message: error.localizedDescription))
            case . success(let url):
                switch await session.post(url: URL(string: url)!, parameters: parameters) {
                    case .failure(let error): return Result.failure(NautaException.changePasswordException(message: error.localizedDescription))
                    case .success(let data): return Result.success(String(data: data, encoding: .utf8) ?? "")
    ...
    public func changeEmailPassword(oldPassword: String, newPassword: String) async -> Result<String, Error> {
        let parameters = [
            "csrf": self.csrf,
            "old_password": oldPassword,
            "new_password": newPassword,
    ...
    public func getConnectionsSummary(year: Int, month: Int) async -> Result<String, Error> {
        ...
        switch makeUrl(action: Action.GetConnections, getAction: true, subAction: "summary") {
            case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
            case . success(let url):
                switch await session.post(url: URL(string: url)!, parameters: parameters) {
                    case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
                    case .success(let data): return Result.success(String(data: data, encoding: .utf8) ?? "")
    ...
    public func getConnections(connectionsSummary: ConnectionsSummary, large: Int = 0, reversed: Bool = false) async -> Result<[String], Error> {
        var connections = [String]()
        let internalLarge = large == 0 || large > connectionsSummary.count ? connectionsSummary.count : large
        ...
        if connectionsSummary.count != 0 {
            ...
            while ((connections.count * 14) - rest) < internalLarge && currentPage >= 1 {
                ...
                switch makeUrl(action: Action.GetConnections, getAction: true, subAction: "list", yearMonthSelected: connectionsSummary.yearMonthSelected, count: connectionsSummary.count, page: page) {
                    case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
                    case . success(let url):
                        switch await session.get(url: URL(string: url)!) {
                            case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
    ...
    public func getRecharges(rechargesSummary: RechargesSummary, large: Int, reversed: Bool) async -> Result<[String], Error> {
        var recharges = [String]()
        let internalLarge = large == 0 || large > rechargesSummary.count ? rechargesSummary.count : large
        ...
        if rechargesSummary.count != 0 {
            ...
            while ((recharges.count * 14) - rest) < internalLarge && currentPage >= 1 {
                let page = currentPage != 1 ? currentPage : nil
                switch makeUrl(action: Action.GetRecharges, getAction: true, subAction: "list", yearMonthSelected: rechargesSummary.yearMonthSelected, count: rechargesSummary.count, page: page) {
                    case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
                    case . success(let url):
                        switch await session.get(url: URL(string: url)!) {
                            case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
    ...
    public func getTransfers(transfersSummary: TransfersSummary, large: Int, reversed: Bool) async -> Result<[String], Error> {
        var transfers = [String]()
        let internalLarge = large == 0 || large > transfersSummary.count ? transfersSummary.count : large
        ...
        if transfersSummary.count != 0 {
            ...
            while ((transfers.count * 14) - rest) < internalLarge && currentPage >= 1 {
                let page = currentPage != 1 ? currentPage : nil
                switch makeUrl(action: Action.GetTransfers, getAction: true, subAction: "list", yearMonthSelected: transfersSummary.yearMonthSelected, count: transfersSummary.count, page: page) {
                    case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
                    case . success(let url):
                        switch await session.get(url: URL(string: url)!) {
                            case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
    ...
    public func getQuotesPaid(quotesPaidSummary: QuotesPaidSummary, large: Int, reversed: Bool) async -> Result<[String], Error> {
        var quotesPaid = [String]()
        let internalLarge = large == 0 || large > quotesPaidSummary.count ? quotesPaidSummary.count : large
        ...
        if quotesPaidSummary.count != 0 {
            ...
            while ((quotesPaid.count * 14) - rest) < internalLarge && currentPage >= 1 {
                let page = currentPage != 1 ? currentPage : nil
                switch makeUrl(action: Action.GetQuotesPaid, getAction: true, subAction: "list", yearMonthSelected: quotesPaidSummary.yearMonthSelected, count: quotesPaidSummary.count, page: page) {
                    case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
                    case . success(let url):
                        switch await session.get(url: URL(string: url)!) {
                            case .failure(let error): return Result.failure(NautaException.getInformationException(message: error.localizedDescription))
