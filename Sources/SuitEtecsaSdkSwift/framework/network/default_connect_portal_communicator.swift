...
public class DefaultConnectPortalCommunicator : ConnectPortalCommunicator {
    ...
    public var dataSession: DataSession = DataSession(username: "", csrfHw: "", wlanUserIp: "", attributeUUID: "")
    ...
    public var remainingTime: Result<String, Error> {
        get async {
            ...
            switch await session.post(url: URL(string: makeUrl(action: Action.LoadUserInformation))!, parameters: parameters) {
    ...
    public func getNautaConnectInformation(parameters: [String : String]) async -> Result<String, Error> {
        switch await session.post(url: URL(string: makeUrl(action: Action.LoadUserInformation))!, parameters: parameters) {
            case .failure(let error): return Result.failure(error)
            case .success(let data): return Result.success(String(data: data, encoding: .utf8) ?? "")
        }
    ...
    public func disconnect() async -> Result<String, Error> {
        ...
        switch await session.get(url: URL(string: makeUrl(action: Action.Logout, parameters: parameters))!) {
            case .failure(let error): return Result.failure(error)
            case .success(let data): return Result.success(String(data: data, encoding: .utf8) ?? "")
        }
    }
