...
public class NautaApi {
    ...
    public func login(captchaCode: String) async throws -> NautaUser {
        switch await userPortalCommunicator.login(username: credentials.username, password: credentials.password, captchaCode: captchaCode) {
            case .success(let html):
    ...
    public func disconnect() async throws {
        switch await connectPortalCommunicator.disconnect() {
            case .failure(let error): throw error
            case .success(let html):
                if connectPortalScraper.isSuccessLogout(html: html) { break } else { 
    ...
    public func transfer(amount: Float, destinationAccount: String) async throws {
        ...
        switch await userPortalCommunicator.transfer(amount: amount, password: credentials.password, destinationAccount: destinationAccount) {
            case .failure(let error): throw error
            case .success(let html):
                switch userPortalScraper.parseErrors(html: html) {
                    case .failure(let error): throw error
    ...
    public func payNautaHome(amount: Float) async throws {
        ...
        switch await userPortalCommunicator.transfer(amount: amount, password: credentials.password, destinationAccount: nil) {
            case .failure(let error): throw error
            case .success(let html):
                switch userPortalScraper.parseErrors(html: html) {
                    case .failure(let error): throw error
    ...
    public func changePassword(newPassword: String) async throws {
        ...
        switch await userPortalCommunicator.changePassword(oldPassword: credentials.password, newPassword: newPassword) {
            case .failure(let error): throw error
            case .success(let html):
                switch userPortalScraper.parseErrors(html: html) {
                    case .failure(let error): throw error
    ...
    public func changeEmailPassword(oldPassword: String, newPassword: String) async throws {
        ...
        switch await userPortalCommunicator.changeEmailPassword(oldPassword: oldPassword, newPassword: newPassword) {
            case .failure(let error): throw error
            case .success(let html):
                switch userPortalScraper.parseErrors(html: html) {
                    case .failure(let error): throw error
    ...
    public func getConnections(connectionsSummary: ConnectionsSummary, large: Int = 0, reversed: Bool = false) async throws -> [Connection] {
        var connections = [Connection]()

        switch await userPortalCommunicator.getConnections(connectionsSummary: connectionsSummary, large: large, reversed: reversed) {
            case .failure(let error): throw error
    ...
    public func getRecharges(rechargessSummary: RechargesSummary, large: Int = 0, reversed: Bool = false) async throws -> [Recharge] {
        var recharges = [Recharge]()

        switch await userPortalCommunicator.getRecharges(rechargesSummary: rechargessSummary, large: large, reversed: reversed) {
            case .failure(let error): throw error
            case .success(let actionsRaw):
                try actionsRaw.forEach { html in 
    ...
    public func getTransfers(transfersSummary: TransfersSummary, large: Int = 0, reversed: Bool = false) async throws -> [Transfer] {
        var transfers = [Transfer]()

        switch await userPortalCommunicator.getTransfers(transfersSummary: transfersSummary, large: large, reversed: reversed) {
            case .failure(let error): throw error
            case .success(let actionsRaw):
                try actionsRaw.forEach { html in 
    ...
    public func getQuotesPaid(quotesPaidSummary: QuotesPaidSummary, large: Int = 0, reversed: Bool = false) async throws -> [QuotePaid] {
        var quotesPaid = [QuotePaid]()

        switch await userPortalCommunicator.getQuotesPaid(quotesPaidSummary: quotesPaidSummary, large: large, reversed: reversed) {
            case .failure(let error): throw error
            case .success(let actionsRaw):
                try actionsRaw.forEach { html in 
